...
int main(void) {
    ...
    char* queue_string = malloc(10000);
    PQ_toString(readyQ, queue_string);
    fprintf(output, "%s\n", queue_string);
    free(queue_string);

    fprintf(output, "Waiting Queue1 Contents:\n");
    int queue_string_size = FIFOq_toString_size(waitQueue1); // this block prints new Queue
...
void run_scheduler(Interrupt interrupt_type) {
    ...
    case timer_interrupt:
        ...
        if (current_process != idle_process) {
            PQ_enqueue(readyQ, current_process);
        }
    ...
    case io1_interrupt:
        ...
        PQ_enqueue(readyQ, finished_pcb);
...
void run_dispatcher() {
    ...
    if (PQ_is_empty(readyQ)) {
        current_process = idle_process;
    } else {
        current_process = PQ_dequeue(readyQ);
    }
